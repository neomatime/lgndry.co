(function () {
  ...
  var AUTH_CALLBACK_URL = "https://www.lgndry-co.co.za/auth-callback.html";
  ...
  var resend = document.querySelector("[data-resend-form]");
  if (resend) {
    var requestedEmail = params.get("email") || "";
    resend.elements.email.value = requestedEmail;
    resend.addEventListener("submit", function (event) {
      event.preventDefault();
      setBusy(resend, true);
      feedback("[data-resend-feedback]", "");
      client.auth.resend({ type: "signup", email: resend.elements.email.value.trim(), options: { emailRedirectTo: AUTH_CALLBACK_URL } }).then(function (result) {
        if (result.error) throw result.error;
        feedback("[data-resend-feedback]", "A new verification email has been sent. Use only the newest link.", true);
        setBusy(resend, false);
      }).catch(function (error) {
        feedback("[data-resend-feedback]", error.message);
        setBusy(resend, false);
      });
    });
  }
