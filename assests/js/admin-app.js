(function () {
  ...
  function detailView(schema, record) {
    ...
    return '<section class="record-detail">' +
      ...
      orderProgress + '<div class="detail-layout"><div class="detail-main"><form class="detail-editor" action="https://www.lgndry-co.co.za/admin.html" method="post" autocomplete="on" data-detail-form><div class="detail-section-head"><div><span class="eyebrow">Record information</span><h3>Edit ' + esc(detailType(schema)) + '</h3></div><span>Changes sync to the studio database</span></div><div class="form-grid detail-form-grid">' + schema.fields.map(function (field) { return renderField(field, record[field.name]); }).join("") + '</div><div class="detail-savebar"><span data-detail-save-status>Review changes before saving.</span><button class="primary-action" type="submit">Save Changes</button></div></form></div>' +
  ...
  function bindSettings() {
    ...
    function refreshTeamList() {
      ...
      Promise.all([
        callTeamFunction({ action: "list" }),
        supabaseClient.from("admin_users").select("user_id")
      ]).then(function (results) {
        var data = results[0] || {};
        var adminResult = results[1] || {};
        if (adminResult.error) throw adminResult.error;
        var adminIds = new Set((adminResult.data || []).map(function (record) { return record.user_id; }));
        var users = (data.users || []).filter(function (user) { return adminIds.has(user.id); });
